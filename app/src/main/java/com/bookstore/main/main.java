...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (running) {
            ...
            switch (choice) {
                case 1:
                    if (hasPermission("book_management")) {
                        bookManagementMenu();
                    }
                    break;
                case 2:
                ...
                case 3:
                    ...
                    break;
                case 4:
                    if (hasPermission("algorithms")) {
                        demonstrateAlgorithms();
                    }
                    break;
                case 5:
                    if (hasPermission("queue_processing")) {
                        processOrderQueue();
                    }
                    break;
                case 6:
    ...
    private static void displayMainMenu() {
        ...
        System.out.println("4. Demonstrate Algorithms");
        System.out.println("5. Process Order Queue");
        if (authService.isCurrentUserAdmin()) {
            System.out.println("6. User Management (Admin)");
        }
    ...
    private static void initializeSystem() {
        ...
        if (!DatabaseTestUtil.testDatabaseConnection()) {
            ...
            System.err.println("\nYou can use the database setup scripts in the 'database' folder.");
            System.exit(1);
        }
    ...
    private static boolean hasPermission(String operation) {
        ...
        // For now, all logged-in users have access to basic operations
        // Admins have access to everything, users have limited access
        User currentUser = authService.getCurrentUser();
    ...
    private static void createNewUser() {
        ...
        if (authService.registerUser(username, tempPassword, email, firstName, lastName, role)) {
            System.out.println("User created successfully!");
            System.out.println("Temporary password: " + tempPassword);
            System.out.println("Please ask the user to change their password on first login.");
        } else {
            System.out.println("Failed to create user. Username or email might already exist.");
        }
    }
    ...
    private static void deactivateUser() {
        ...
        try {
            ...
            List<User> activeUsers = users.stream()
                    ...
                    .filter(u -> u.getUserId() != authService.getCurrentUser().getUserId()) // Can't deactivate self
                    .toList();
            
            if (activeUsers.isEmpty()) {
                System.out.println("No other active users found.");
                return;
            }
            ...
            int userId = getIntInput("Enter User ID to deactivate (0 to cancel): ");
            if (userId == 0) {
                return;
            }
            
            User userToDeactivate = activeUsers.stream()
            ...
            System.out.print("Are you sure you want to deactivate user '" + userToDeactivate.getUsername() + "'? (y/N): ");
            String confirm = scanner.nextLine();
            
            if (confirm.toLowerCase().startsWith("y")) {
                if (authService.getUserDAO().deactivateUser(userId)) {
